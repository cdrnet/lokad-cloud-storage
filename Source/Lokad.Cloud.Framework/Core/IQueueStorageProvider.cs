...
namespace Lokad.Cloud.Core
{
	...
	public interface IQueueStorageProvider
	{
		...
		/// <param name="queueId">Identifier of the queue to be pulled.</param>
		/// <param name="count">Maximal number of messages to be retrieved.</param>
		/// <returns>Enumeration of messages, possibly empty.</returns>
		IEnumerable<T> Get<T>(string queueId, int count);

		/// <summary>Put message on a queue.</summary>
		/// <typeparam name="T">Type of the messages.</typeparam>
		/// <param name="queueId">Identifier of the queue where messages are put.</param>
		/// <param name="messages">Messages to be put.</param>
		void Put<T>(string queueId, IEnumerable<T> messages);

		/// <summary>Deletes messages from a queue.</summary>
		/// <typeparam name="T">Type of the messages.</typeparam>
		/// <param name="queueId">Identifier of the queue where the messages are removed from.</param>
		...
		void Delete<T>(string queueId, IEnumerable<T> messages);
	}
}
